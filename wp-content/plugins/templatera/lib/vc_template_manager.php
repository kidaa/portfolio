...
if ( ! class_exists( 'VcTemplateManager' ) ) {
	class VcTemplateManager {
		...
		function init() {
			...
			if ( ( isset( $_GET['post'] ) && $this->isSamePostType( get_post_type( $_GET['post'] ) ) ) || ( isset( $_GET['post_type'] ) && $this->isSamePostType( $_GET['post_type'] ) ) ) {
...
HTML;
		...
		public function isValidPostType() {
			return in_array( get_post_type(), array_merge( vc_editor_post_types(), array( 'templatera' ) ) );
		}
